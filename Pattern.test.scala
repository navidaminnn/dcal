...
class PatternTests extends munit.FunSuite:
  ...
  extension [T](pat: on[T])
    def onTree(using munit.Location)(name: String)(tree: Node.All)(
        expected: T
    ): pat.type =
      test(name):
    ...
    def onChildren(using munit.Location)(name: String)(children: Node.Child*)(
      ...
      then
        test(name):
          val result =
            atHandle(Manip.Handle.Sentinel(top, 0))(pat.value).perform()
          assertEquals(result, expected)
        pat
      else onTree(name)(children.head)(expected)

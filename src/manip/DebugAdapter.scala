...
object DebugAdapter:
  ...
  final class State:
    ...
    var currHandle: Option[Handle] | Null = null
    ...
    def refOfHandle(handle: Handle): Int =
      handle match
        case Handle.AtChild(_, _, child) => refOf(child)
        case Handle.AtTop(top)           => refOf(top)
        case Handle.Sentinel(parent, _)  => refOf(parent)
